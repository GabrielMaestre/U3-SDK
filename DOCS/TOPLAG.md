...
- `Camera.Render`: `52,7%`; `Render.OpaqueGeometry`: `40,2%`, `397` draw calls e `2,049 ms` de GPU.
- `RenderDeferred.GBuffer`: `26,8%`; `BatchRenderer.Flush`: `26,6%`; `Batch.DrawStatic`: `21,7%`, `63` draws e `1,109 ms` de GPU.
- Interpretação: maior custo é preparar, cullar e submeter renderers opacos. Static batching já consolida geometria; migrar indiscriminadamente para GPU instancing pode perder batching. Primeiro controle aplicado reduz far clip no Editor para remover trabalho, não transferi-lo.
...
| 3 | Pipeline único de visibilidade e LOD | Reutilizar regiões para frustum/occlusion culling, LOD e distância por categoria; aplicar instancing, impóstores e orçamento separado para folhagem, terreno, água, luzes e sombras. | GPU, VRAM e CPU de render | `M/E/C`: captura do Editor mede `Render.OpaqueGeometry` em `40,2%` da CPU e GPU em `5,09 ms`; modo leve limita far clip a `768 m` somente no Editor. Captura standalone e Frame Debugger ainda precisam identificar meshes/materiais dominantes. |
